...
const SettingsModal: React.FC<{ isOpen: boolean; onClose: () => void; }> = ({ isOpen, onClose }) => {
    ...
    const [logoUrl, setLogoUrl] = useState(localStorage.getItem('pompom_logo') || '/logo.png');
    const [themeColor, setThemeColor] = useState(localStorage.getItem('pompom_theme') || 'rose');
    
    // Apply theme
    useEffect(() => {
        const root = document.documentElement;
        const themes: Record<string, any> = {
            rose: { '600': '#e11d48', '500': '#f43f5e', '100': '#ffe4e6', '50': '#fff1f2' },
            blue: { '600': '#2563eb', '500': '#3b82f6', '100': '#dbeafe', '50': '#eff6ff' },
            green: { '600': '#059669', '500': '#10b981', '100': '#d1fae5', '50': '#ecfdf5' },
            purple: { '600': '#7c3aed', '500': '#8b5cf6', '100': '#ede9fe', '50': '#f5f3ff' },
            orange: { '600': '#ea580c', '500': '#f97316', '100': '#ffedd5', '50': '#fff7ed' },
        };
        const selected = themes[themeColor];
        if(selected) {
             Object.keys(selected).forEach(k => {
                 root.style.setProperty(`--brand-${k}`, selected[k]);
             });
        }
        localStorage.setItem('pompom_theme', themeColor);
        localStorage.setItem('pompom_logo', logoUrl);
    }, [themeColor, logoUrl]);
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-scale-up">
                ...
                <div className="flex border-b">
                    <button onClick={() => setActiveTab('geral')} className={`flex-1 py-3 text-sm font-bold ${activeTab === 'geral' ? 'text-brand-600 border-b-2 border-brand-600' : 'text-gray-500'}`}>Geral</button>
                    <button onClick={() => setActiveTab('aparencia')} className={`flex-1 py-3 text-sm font-bold ${activeTab === 'aparencia' ? 'text-brand-600 border-b-2 border-brand-600' : 'text-gray-500'}`}>Aparência</button>
                </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <div className="flex gap-2">
                                    <input value={logoUrl} onChange={e => setLogoUrl(e.target.value)} className="flex-1 border p-2 rounded-lg text-sm" />
                                    <div className="w-10 h-10 border rounded-lg overflow-hidden flex items-center justify-center bg-gray-50"><img src={logoUrl} alt="Preview" className="w-full h-full object-contain" onError={(e) => (e.target as HTMLImageElement).src = 'https://placehold.co/40'} /></div>
                                </div>
                                <p className="text-[10px] text-gray-400 mt-1">Cole o link da imagem (ex: /logo.png ou https://...)</p>
                            </div>
                        </div>
                    )}
                    {activeTab === 'aparencia' && (
                        <div className="grid grid-cols-2 gap-3">
                            {['rose', 'blue', 'green', 'purple', 'orange'].map(c => (
                                <button key={c} onClick={() => setThemeColor(c)} className={`p-4 rounded-xl border-2 flex items-center justify-between transition ${themeColor === c ? 'border-gray-800 bg-gray-50' : 'border-gray-100 hover:border-gray-300'}`}>
                                    <span className="capitalize font-bold text-gray-700">{c}</span>
                                    <div className={`w-6 h-6 rounded-full`} style={{backgroundColor: c === 'rose' ? '#e11d48' : c === 'blue' ? '#2563eb' : c === 'green' ? '#059669' : c === 'purple' ? '#7c3aed' : '#ea580c'}} />
                                </button>
                            ))}
                        </div>
                    )}
                ...
                <div className="p-4 bg-gray-50 text-right">
                    <button onClick={onClose} className="px-6 py-2 bg-brand-600 text-white rounded-lg font-bold text-sm">Concluir</button>
                </div>
            </div>
        </div>
    )
...
const RevenueView: React.FC<{ appointments: Appointment[]; services: Service[]; clients: Client[]; }> = ({ appointments, services, clients }) => {
    ...
    const getWeeklyChartData = () => {
        ...
        businessDays.forEach(dayIndex => {
            ...
            // Use local date string comparison to avoid UTC issues
            ...
            const dailyApps = appointments.filter(a => { 
                ...
                // Convert appointment UTC ISO string to local YYYY-MM-DD
    ...
    const dailyApps = appointments.filter(a => {
        ...
        const aMonth = String(aDate.getMonth() + 1).padStart(2, '0');
        const aDay = String(aDate.getDate()).padStart(2, '0');
        return `${aYear}-${aMonth}-${aDay}` === selectedDate;
    });
...
const ScheduleManager: React.FC<{ appointments: Appointment[]; clients: Client[]; services: Service[]; onAdd: (app: Appointment, client: Client, pet: Pet, services: Service[], duration: number) => void; onEdit: (app: Appointment, client: Client, pet: Pet, services: Service[], duration: number) => void; onUpdateStatus: (id: string, status: Appointment['status']) => void; onDelete: (id: string) => void; googleUser: GoogleUser | null; }> = ({ appointments, clients, services, onAdd, onEdit, onUpdateStatus, onDelete }) => {
    ...
    const AppointmentCard = ({ app, onClick }: { app: Appointment, onClick?: () => void }) => { 
        ...
        else if (allServiceNames.includes('higi')) colorClass = 'bg-yellow-100 border-yellow-300 text-yellow-900'; 
...
const App: React.FC = () => {
  ...
  const handleSyncAppointments = async (token: string, silent = false) => {
      ...
      try {
          ...
          rows.forEach((row: string[], idx: number) => {
              ...
              // O ID sheet_idx deve corresponder ao índice do array rows para facilitar a edição/exclusão
  ...
  const handleEditAppointment = async (app: Appointment, client: Client, pet: Pet, appServices: Service[], manualDuration: number) => {
    ...
    if (accessToken && app.id.startsWith('sheet_')) {
        ...
        // Correção para edição: Os dados começam na linha 5. O índice 0 no array corresponde à linha 1, mas nosso loop ignora < 4.
        // Se o ID é sheet_5, isso significa que era o item no índice 5 do array total de linhas.
        // Linha 1 = index 0. Linha 5 = index 4.
        // Se o sheet_idx vem do 'rows.forEach((row, idx))', idx é absoluto em relação à planilha.
        // Então rowNumber deve ser idx + 1 (Google Sheets API usa índice baseado em 1).
     ...
     if (appToDelete && accessToken) {
         ...
         if (appToDelete.id.startsWith('sheet_')) {
             try {
                 ...
                 const row = idx + 1; // 1-based index based on the original row index from sync
