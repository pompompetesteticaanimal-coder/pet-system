...
const App: React.FC = () => {
  ...
  const handleEditAppointment = async (app: Appointment, c: Client, p: Pet, ss: Service[]) => { if(accessToken && app.googleEventId) { const desc = ss.map(s=>s.name).join(' + '); let dur = 0; ss.forEach(s=>dur+=(s.durationMin||0)); await googleService.updateEvent(accessToken, app.googleEventId, { summary: `Banho/Tosa: ${p.name}`, description: `Serviços: ${desc}\nObs: ${p.notes}`, startTime: app.date, durationMin: dur }); } if(accessToken && app.id.startsWith('sheet_')) { const row = parseInt(app.id.split('_')[1])+2; const d = new Date(app.date); let dur = 0; ss.forEach(s=>dur+=(s.durationMin||0)); await googleService.updateSheetValues(accessToken, SHEET_ID, `Agendamento!A${row}:O${row}`, [p.name, c.name, c.phone, `${c.address} ${c.complement}`, p.breed, p.size, p.coat, ss[0]?.name||'', ss[1]?.name||'', ss[2]?.name||'', ss[3]?.name||'', d.toLocaleDateString('pt-BR'), d.toLocaleTimeString('pt-BR', {hour:'2-digit', minute:'2-digit'}), p.notes, dur]); } const u = appointments.map(a => a.id === app.id ? app : a); setAppointments(u); db.saveAppointments(u); alert('Atualizado!'); };
